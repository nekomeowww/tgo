...
import (
	...
	"github.com/nekomeowww/tgo/pkg/locales"
...
func NewI18n(options ...NewI18nOption) (*I18n, error) {
	...
	if opts.logger == nil {
		logger, err := logger.NewLogger(logger.WithLevel(zapcore.DebugLevel), logger.WithAppName("i18n"))
		if err != nil {
			return nil, err
		}
	...
	if opts.localesDir != "" {
		...
		for _, v := range dirs {
			...
			err = bundle.AddMessages(language.MustParse(filepath.Base(v.Name())), file.Messages...)
			if err != nil {
				return nil, err
			}
		}
